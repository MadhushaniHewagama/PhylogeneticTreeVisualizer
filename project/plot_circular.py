...
def create_circular_tree():
    ...
    for clade in tree.find_clades(order=traverse_order):
        if clade.name and clade.branch_length and clade.confidence is None:
            tooltip.append(
                f" {clade.name}")       
        else:
            tooltip.append('')
    ...
    py.plot(fig, filename='circular_phyloxml',auto_open=False)

    tls.get_embed('https://plot.ly/~madhushani/2/phylogenetic-tree/')